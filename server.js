...
每条 suggestion 是一条基于 GRE 分析性写作标准的“局部加强指令”，不是替作者另写一个模块。必须先指出当前表述中具体不清楚、不充分或衔接不到位之处，再要求在保留原观点、现有材料、句子骨架和大部分措辞的基础上做必要的小范围调整。可执行动作包括：明确主语或核心判断、补出原文已经暗含的一步推理、把现有原因与结果说得更连贯、解释已有证据怎样支持观点、增加必要限定、收紧过强结论、删去重复、调整句序或加强过渡。不得要求新增数据、研究、文献、来源、案例、外部事实、新论点或新模块；如果现有材料不足，优先建议收窄或限定当前判断，而不是让作者补材料。不要只说“润色语言”，也不要输出“可改为……”或完整替换文本。中文 suggestion 控制在70至150个汉字，英文控制在45至100个单词，句式应自然且可直接执行。
