...
- 按 GRE 分析性写作的标准，论点是否清楚、立场是否稳定、范围与限定是否准确；
- 原因是否在现有表述内把“为什么”说清楚，推理步骤是否连贯；
- 已有证据或例子是否被正确解释，并与观点建立了明确联系，相关性是否被误写成因果性；
- 反论是否直接回应核心论点，现有回应是否清楚且有必要限定；
- 结论是否准确收束前文已经出现的内容，而没有遗漏关键判断或加入新主张；
- 句子之间是否存在逻辑跳跃、指代含混、概念偷换、重复、过度概括或表达不够精确。
...
每条 suggestion 是一段基于 GRE 分析性写作标准的、完整连贯的“局部加强指令”，不是替作者另写一个模块。每段必须把三层意思说清楚：第一，准确指出当前哪一句判断、哪一步推理或哪一组模块关系存在什么具体不足，以及这种不足为什么会影响读者理解或论证成立；第二，明确说明应当怎样调整现有表述，修改动作必须能直接执行；第三，结合本条实际问题说明调整后会让哪一处逻辑、边界、解释或回应变得更清楚、更充分或更有说服力，不能只用“增强论证”作空泛结尾。三层意思要自然组织成一段，不要写成标签、提纲、短语串或互不衔接的命令，也不要让所有建议机械套用“由于……所以……”的同一句式。
修改必须保留原观点、现有材料、句子骨架和大部分措辞，只做必要的小范围调整。可执行动作包括：明确主语或核心判断、补出原文已经暗含的一步推理、把现有原因与结果说得更连贯、解释已有证据怎样支持观点、增加必要限定、收紧过强结论、删去重复、调整句序或加强过渡。不得要求新增数据、研究、文献、来源、案例、外部事实、新论点或新模块；如果现有材料不足，优先建议收窄或限定当前判断，而不是让作者补材料。不要只说“润色语言”，也不要输出“可改为……”或完整替换文本。中文 suggestion 控制在120至220个汉字，英文控制在80至150个单词；表达完整优先，不得为了压缩长度省略“问题—动作—作用”中的任何一层。
...
4. 按 GRE 分析性写作的标准从现有文本内部把关：立场是否明确、分析是否到位、原因是否在当前材料内得到充分说明、已有证据是否被解释并连接到观点、反论是否直接回应、结论是否准确收束、概念和限定是否精确、前后是否存在跳跃。
5. 建议必须是局部加强：明确某个判断、补出原文暗含的推理连接、解释已有材料的作用、增加限定、收窄过强表述、删除重复、调整句序或加强过渡。保留原模块的核心内容、句子骨架和大部分措辞，不要建议整段重写。
6. 不得要求或虚构新的数据、研究、文献、来源、案例、外部事实、新论点或新模块。现有材料不足时，建议限定当前判断，而不是补充外部材料。`;
