 ...
 *
 * 正常语义模块本身使用 6px 的右侧间距；长度拉伸时也保持同一个
 * 视觉节奏，不能因为短尾换行分支额外放大成 14px。
 * 圆点直径为 8px，且以模块末端为中心覆盖，因此 6px 已能在圆点
 * 右半径之外留下约 2px 的净空，同时与其他模块的间距保持一致。
 * “短尾巴”问题应由换行/右对齐判断解决，而不是通过增大 gap 兜底。
 */
const LENGTH_PREVIEW_END_GAP = 6;
